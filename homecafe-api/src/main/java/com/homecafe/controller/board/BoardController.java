...
import com.homecafe.service.board.dto.request.*;
...
public class BoardController {
	...
	public ApiResponse<List<BoardWithCreatorInfoResponse>> retrieveBoardList(@Valid RetrieveBoardListRequest request,  @LoginMember Long memberId) {
		return ApiResponse.of(boardService.retrieveBoardList(request.getLastBoardId(), request.getSize(), memberId));
	}
	...
	public ApiResponse<List<BoardWithCreatorInfoResponse>> retrieveSearchBoardList(@Valid RetrieveSearchBoardListRequest request,  @LoginMember Long memberId) {
		return ApiResponse.of(boardService.retrieveSearchBoardList(request.getKeyword(), request.getLastBoardId(), request.getSize(), memberId));
	}

	@Operation(summary = "특정 피드의 상세 조회 API")
	@GetMapping("/api/v1/board")
	public ApiResponse<BoardWithCommentInfoResponse> retrieveBoard(@Valid BoardIdRequest request, @LoginMember Long memberId) {
		return ApiResponse.of(boardService.retrieveBoard(request.getBoardId(), memberId));
	}
	...
	public ApiResponse<String> deleteBoard(@Valid BoardIdRequest request, @LoginMember Long memberId) {
		boardService.deleteBoard(request.getBoardId(), memberId);
		return ApiResponse.OK;
	}

	@Operation(summary = "피드의 좋아요를 추가하는 API", description = "Bearer 토큰이 필요합니다")
	@PostMapping("/api/v1/board/like")
	public ApiResponse<String> addBoardLike(@Valid @RequestBody AddBoardLikeRequest request, @LoginMember Long memberId) {
		boardService.addBoardLike(request.getBoardId(), memberId);
		return ApiResponse.OK;
	}

	@Operation(summary = "피드의 좋아요를 취소하는 API", description = "Bearer 토큰이 필요합니다")
	@DeleteMapping("/api/v1/board/like")
	public ApiResponse<String> cancelBoardLike(@Valid BoardIdRequest request, @LoginMember Long memberId) {
		boardService.cancelBoardLike(request.getBoardId(), memberId);
		return ApiResponse.OK;
	}
